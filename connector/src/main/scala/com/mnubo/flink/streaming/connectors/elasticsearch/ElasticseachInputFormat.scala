...
class ElasticseachInputFormat[T](nodes: Set[String],
                                 port: Int,
                                 index: String,
                                 query: String,
                                 deserializer: RecordMarshaller[T]) extends InputFormat[T, ElasticsearchInputSplit] {
  @transient
  ...
  private var currentSplit: ElasticsearchInputSplit = null
  ...
  override def configure(parameters: Configuration) = {
    ...
    if (log == null)
      log = LoggerFactory.getLogger(classOf[ElasticseachInputFormat[T]])
    ...
    val resVal =
      executeHttpQuery(getOnOneNode(s"/$index/_search_shards"))
        ...
        .map { shardReplicasJsonArray =>
          ...
          ElasticsearchInputSplit(index, shard, nodeIps(nodeId), nodeId, port)
        }
       .toArray
  ...
  override def getInputSplitAssigner(inputSplits: Array[ElasticsearchInputSplit]): InputSplitAssigner = new InputSplitAssigner {
    var i = -1
  ...
  override def open(split: ElasticsearchInputSplit) = {
